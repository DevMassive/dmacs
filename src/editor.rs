...
use crate::editor::clipboard::ClipboardManager;
...
impl Editor {
    ...
    pub fn execute_action(&mut self, action: Action) -> Result<()> {
        ...
        if !matches!(action, Action::KillLine) {
            self.clipboard_manager.last_action_was_kill = false;
        }
    ...
    pub fn undo(&mut self) {
        if let Some(mut actions_to_undo) = self.undo_manager.undo_stack.pop() {
            ...
            for action_diff in actions_to_undo.iter() {
                match self.document.apply_action_diff(action_diff, true) {
                    ...
                    Err(e) => {
                        ...
                        self.undo_manager.undo_stack.push(actions_to_undo);
                        return;
                    }
                }
            }
            actions_for_redo.reverse();
            self.undo_manager.redo_stack.push(actions_for_redo);
    ...
    pub fn redo(&mut self) {
        if let Some(actions_to_redo) = self.undo_manager.redo_stack.pop() {
            ...
            for action_diff in actions_to_redo.iter() {
                match self.document.apply_action_diff(action_diff, false) {
                    ...
                    Err(e) => {
                        ...
                        self.undo_manager.redo_stack.push(actions_to_redo);
                        return;
                    }
                }
            }
            self.undo_manager.undo_stack.push(actions_for_undo);
    ...
    pub(super) fn commit(&mut self, action_type: LastActionType, action_diff: &ActionDiff) {
        self.undo_manager.save_state_for_undo(action_type);
        self.undo_manager.add_to_undo_group(action_diff);
    ...
    pub fn kill_line(&mut self) -> Result<()> {
        ...
        if x < current_line_len {
            killed_text = self.document.lines[y][x..].to_string();
            self.commit(
                LastActionType::Deletion,
                &ActionDiff {
                    cursor_start_x: self.cursor_x,
                    ...
                    end_y: self.cursor_y,
                    new: vec![],
                    old: vec![killed_text.clone()],
                },
            );
        } else {
            self.delete_forward_char()?;
            killed_text = "\x0a".to_string();
        }

        if let Err(e) = self.clipboard_manager.kill_text(&killed_text) {
    ...
    pub fn yank(&mut self) -> Result<()> {
        let text_to_yank = self.clipboard_manager.get_yank_text().to_string();
        ...
        let last_yank_line_len = yank_lines.last().unwrap().len();
        ...
        if line_count > 1 {
            self.commit(
                ...
                &ActionDiff {
                    cursor_start_x: self.cursor_x,
                    cursor_start_y: self.cursor_y,
                    cursor_end_x: last_yank_line_len,
                    cursor_end_y: self.cursor_y + line_count - 1,
                    start_x: self.cursor_x,
                    start_y: self.cursor_y,
                    end_x: last_yank_line_len,
            ...
            self.commit(
                ...
                &ActionDiff {
                    cursor_start_x: self.cursor_x,
                    cursor_start_y: self.cursor_y,
                    cursor_end_x: self.cursor_x + last_yank_line_len,
                    cursor_end_y: self.cursor_y,
                    start_x: self.cursor_x,
                    start_y: self.cursor_y,
                    end_x: self.cursor_x + last_yank_line_len,
                    end_y: self.cursor_y,
                    new: yank_lines,
                    old: vec![],
                },
    ...
    pub fn _set_clipboard_enabled_for_test(&mut self, enabled: bool) {
        self.clipboard_manager
            ._set_clipboard_enabled_for_test(enabled);
    }
    ...
    pub fn move_line_up(&mut self) {
        ...
        self.commit(
            ...
            &ActionDiff {
                ...
                new: vec![swapped_line1, swapped_line0],
                old: vec![],
            },
        );
    }
    ...
    pub fn scroll_page_down(&mut self) {
        self.scroll
            .scroll_page_down(&mut self.cursor_y, &mut self.cursor_x, &self.document);
    }

    pub fn scroll_page_up(&mut self) {
        self.scroll
            .scroll_page_up(&mut self.cursor_y, &mut self.cursor_x, &self.document);
    }
    ...
    pub fn cut_selection_action(&mut self) -> Result<()> {
        ...
        if let Err(e) = self.clipboard_manager.copy_text(&killed_text) {
            self.status_message = e;
        } else {
            self.status_message = "Selection cut to clipboard.".to_string();
        }
        debug!("Selection cut. Kill buffer: '{killed_text}'");
    ...
    pub fn copy_selection_action(&mut self) -> Result<()> {
        let cursor_pos = self.cursor_pos();
        let copied_text = self.selection.copy_selection(&self.document, cursor_pos)?;
        if let Err(e) = self.clipboard_manager.copy_text(&copied_text) {
            self.status_message = e;
        } else {
            self.status_message = "Selection copied to clipboard.".to_string();
        }
        debug!("Selection copied. Kill buffer: '{copied_text}'");
        Ok(())
    }
    ...
    pub fn set_undo_debounce_threshold(&mut self, threshold_ms: u64) {
        self.undo_manager.set_undo_debounce_threshold(threshold_ms);
    }
